...
class InputField extends Component {
    static defaultProps = {
        ...
        options: [],
        validators: [(event) => { return "Deafault validator running " }, (event) =>{return "ok"}],
    }
    constructor(props) {
        super(props);
        this.state = {
    ...
    async handleChange(event) {
        ...
        this.setState({ errors: this.state.errors }, function () {
            this.props.validators.map(x => {
                this.setState({ errors: [...this.state.errors,x()] })
                // console.log(x())
            })
